...
│    ↓                                             │
│  NautilusGrowth (φ-spaced node expansion)        │
│    ↓                                             │
...
def **init**(self, entropy_threshold: float = 1.5):
...
def **init**(self, tolerance: float = 1.2):
self.vertices = VERTICES
...
class UrgencyGuard:
def **init**(self, p_limit: float = 20.0, t_limit: float = 550.0):
...
def **init**(self, hardware_id: str = “sCO2_Unit_01”,
...
def **init**(self, start_ambient: float = -10.0,
...
class ThermalModel:
def **init**(self, start_t: float = 480.0,
...
class MetaAwareness:
def **init**(self, ambient_floor: float = -48.0,
